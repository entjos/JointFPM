...
2.  The fitted model object needs to be passed to
    [`predict()`](https://rdrr.io/r/stats/predict.html) in order to
    estimate the mean number of events.
...
example. The dataset is included in the
[survival](https://github.com/therneau/survival) package and includes
information on bladder cancer patients receiving three different
treatments: placebo, Pyridoxine, and Thiotepa
...
for the recurrent event. In the example below we use the
[data.table](https://r-datatable.com) package for the data preparation.
